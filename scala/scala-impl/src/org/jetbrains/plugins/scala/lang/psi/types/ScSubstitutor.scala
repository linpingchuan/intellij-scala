  ...
  protected def substInternal(t: ScType): ScType = {
    ...
    val visitor = new ScalaTypeVisitor {
      ...
      override def visitThisType(th: ScThisType): Unit = {
        ...
        def hasSameOrInheritor(thisTp: ScThisType, compound: ScCompoundType): Boolean = {
          compound.components
            .exists {
              _.extractClass
        ...
        @tailrec
        def isSubtype(thisTp: ScThisType, upperBound: ScType): Boolean =
          upperBound.extractDesignated(expandAliases = true) match {
            case Some(t: ScTypeDefinition) =>
              if (isSameOrInheritor(t, thisTp)) true
              else t.selfType match {
                case Some(selfTp) => isSubtype(thisTp, selfTp)
                case _ => false
              }
            case Some(typeParam: PsiTypeParameter) =>
              upperBound match {
                case t: TypeParameterType =>
                  isSubtype(thisTp, t.upperType)
                case p@ParameterizedType(_, _) =>
                  p.designator match {
                    case TypeParameterType(_, _, upperType, _) =>
                      isSubtype(thisTp, p.substitutor.subst(upperType))
                    case _ =>
            ...
            case Some(named: ScTypedDefinition) =>
              isSubtype(thisTp, named.`type`().getOrAny)
            case _ =>
              upperBound match {
                case compound: ScCompoundType =>
                  hasSameOrInheritor(thisTp, compound)
                case _ =>
                  false
              }
          }
        ...
        def doUpdateThisType(thisTp: ScThisType, target: ScType): ScType = {
          if (isSubtype(thisTp, target)) target
          else {
